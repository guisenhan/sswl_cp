...
public class ImageFetcher extends ImageResizer {
    ...
    public static final String HTTP_CACHE_DIR = "cache";  //"http"
	...
	//            Log.d(TAG, "processBitmap - " + data);
	        ...
	        if (f != null) {
	            // Return a sampled down version
	             Bitmap bitmap = decodeSampledBitmapFromFile(f.toString(), mImageWidth, mImageHeight);
	             if (bitmap == null) {
	            	 f.delete();
	             }
	             return bitmap;
...
//        String cacheKey = urlString.substring(urlString.lastIndexOf("/") + 1);
        String cacheKey = CommonUtil.getCacheImgUrl(String.valueOf(urlString));
        final File cacheFile = new File(cache.createFilePath(cacheKey));
        
        if (cache.containsKey(cacheKey)) {
            if (BuildConfig.DEBUG) {
                Log.d(TAG, "downloadBitmap - found in http cache - " + cacheKey);
            }
            return cacheFile;
        }
