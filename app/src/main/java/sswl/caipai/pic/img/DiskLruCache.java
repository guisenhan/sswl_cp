...
//	    String SD_CARD_DIR = Environment.getExternalStorageDirectory() + File.separator + "gz.xiudian";		
	    File dirFile = new File(Constant.CACHE_DIR);
		if (!dirFile.exists()) {
			dirFile.mkdir();  
			Log.i("file", "dirFile mkdir");
		}
		return new File(Constant.CACHE_DIR + File.separator + uniqueName);
	}
